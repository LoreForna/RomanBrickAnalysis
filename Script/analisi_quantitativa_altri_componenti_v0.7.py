"""
SCRIPT ANALISI QUANTITATIVA ALTRI COMPONENTI - VERSIONE 0.7
"""
...
class Analisi(QgsProcessingAlgorithm):
    ...
    def _log_header(self, feedback):
        ...
        feedback.pushInfo("\n" + "="*70)
        feedback.pushInfo("ANALISI QUANTITATIVA ALTRI COMPONENTI - VERSIONE 0.7")
    ...
    def _compute_bounding_boxes(self, layer_base: str, parameters: Dict,
        ...
        field_mapping = self.create_field_mapping([
            ...
            (f'"{FieldNames.AREA_COMPONENTE}"', FieldNames.AREA_COMPONENTE, 6, 6, 4),
            ('"height"', FieldNames.WIDTH_BBOX, 6, 6, 4),
            ('"width"', FieldNames.HEIGHT_BBOX, 6, 6, 4),
            ('"angle"', FieldNames.ANGLE_BBOX, 6, 6, 4),
            ('"perimeter"', FieldNames.PERIMETER_BBOX, 6, 6, 4),
            ('"area"', FieldNames.AREA_BBOX, 6, 6, 4)
        ])
    ...
    ELSE concat(
        round(floor("{FieldNames.WIDTH_BBOX}"/{width_step})*{width_step}, 4),
        ' - ',
        round((floor("{FieldNames.WIDTH_BBOX}"/{width_step})+1)*{width_step}, 4)
    )
END'''
    ...
    ELSE concat(
        round(floor("{FieldNames.HEIGHT_BBOX}"/{height_step})*{height_step}, 4),
        ' - ',
        round((floor("{FieldNames.HEIGHT_BBOX}"/{height_step})+1)*{height_step}, 4)
    )
END'''
    ...
    def _create_rilievo_analysis(self, layer_base: str, bbox_layer: str,
        ...
        field_mapping = self.create_field_mapping([
            ...
            (f'"{FieldNames.SUPERFICIE}"', FieldNames.SUPERFICIE, 10, 0, 0),
            (f'"{FieldNames.AREA_COMPONENTE}"', FieldNames.AREA_COMPONENTE, 6, 6, 4),
            (f'"{FieldNames.WIDTH_BBOX}"', FieldNames.WIDTH_BBOX, 6, 6, 4),
            (f'"{FieldNames.HEIGHT_BBOX}"', FieldNames.HEIGHT_BBOX, 6, 6, 4),
            (f'"{FieldNames.ANGLE_BBOX}"', FieldNames.ANGLE_BBOX, 6, 6, 4),
            (f'"{FieldNames.PERIMETER_BBOX}"', FieldNames.PERIMETER_BBOX, 6, 6, 4),
            (f'"{FieldNames.AREA_BBOX}"', FieldNames.AREA_BBOX, 6, 6, 4)
        ])
    ...
    def _create_campioni_analysis(self, parameters: Dict, stats: Dict,
        ...
        # Calcoli finali (MODIFICATO: rimossi i calcoli dei componenti calcolati)
    ...
    def _create_final_stats_table(self, input_layer: str, context, feedback) -> Dict:
        ...
        field_mapping = self.create_field_mapping([
            ...
            (f'"{FieldNames.USM}"', FieldNames.USM, 10, 0, 0),
            (f'"{FieldNames.CAMPIONE}"', FieldNames.CAMPIONE, 10, 0, 0),
            (f'to_real("{FieldNames.AREA_CAMPIONE}")', 'area campione', 6, 0, 4),
            ('to_int("parz_count")', 'num. componenti parziali', 2, 0, 0),
            ('to_real("parz_sum")', 'totale area componenti parziali', 6, 0, 4),
            ('to_int("int_count")', 'num. componenti interi', 2, 0, 0),
            ('to_real("int_sum")', 'totale area componenti interi', 6, 0, 4),
            ('to_real("int_mean")', 'media area componenti interi', 6, 0, 4),
            ('to_real("width_min")', 'width_min', 6, 0, 4),
            ('to_real("width_max")', 'width_max', 6, 0, 4),
            ('to_real("width_range")', 'width_range', 6, 0, 4),
            ('to_real("width_mean")', 'width_mean', 6, 0, 4),
            ('to_real("width_stddev")', 'width_stddev', 6, 0, 4),
            ('to_real("height_min")', 'height_min', 6, 0, 4),
            ('to_real("height_max")', 'height_max', 6, 0, 4),
            ('to_real("height_range")', 'height_range', 6, 0, 4),
            ('to_real("height_mean")', 'height_mean', 6, 0, 4),
            ('to_real("height_stddev")', 'height_stddev', 6, 0, 4)
        ])
    ...
    def _compute_final_calculations(self, input_layer: str, context, feedback) -> str:
        """Esegue i calcoli finali (aree totali e rapporti) - MODIFICATO"""
        feedback.pushInfo("\n--- CALCOLI FINALI ---")
        ...
        calc1 = processing.run('native:fieldcalculator', {
            ...
            'FIELD_TYPE': 0,
            'FORMULA': 'round(COALESCE("totale area componenti interi", 0) + COALESCE("totale area componenti parziali", 0), 4)',
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        calc2 = processing.run('native:fieldcalculator', {
            ...
            'FIELD_TYPE': 0,
            'FORMULA': 'round(COALESCE("area campione", 0) - COALESCE("totale area componenti", 0), 4)',
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }, context=context, feedback=feedback, is_child_algorithm=True)
...
END''',
    ...
    def _create_output_tables(self, input_layer: str, parameters: Dict,
                             context, feedback, results: Dict):
        """Crea le tabelle di output (tabellare e layer poligonale) - MODIFICATO"""
        feedback.pushInfo("\n--- CREAZIONE TABELLA ANALISI CAMPIONI ---")
        
        # Mapping campi finali per tabella (RIMOSSI i campi calcolati)
        table_fields = self.create_field_mapping([
            (f'"{FieldNames.SITO}"', FieldNames.SITO, 10, 0, 0),
            (f'"{FieldNames.AMBIENTE}"', FieldNames.AMBIENTE, 10, 0, 0),
            (f'"{FieldNames.USM}"', FieldNames.USM, 10, 0, 0),
            (f'"{FieldNames.CAMPIONE}"', FieldNames.CAMPIONE, 10, 0, 0),
            ('"area campione"', 'area_campione', 6, 0, 4),
            ('"num. componenti interi"', 'num_componenti_interi', 2, 0, 0),
            ('"totale area componenti interi"', 'totale_area_componenti_interi', 6, 0, 4),
            ('"media area componenti interi"', 'media_area_componenti_interi', 6, 0, 4),
            ('"num. componenti parziali"', 'num_componenti_parziali', 2, 0, 0),
            ('"totale area componenti parziali"', 'totale_area_componenti_parziali', 6, 0, 4),
            ('"totale area componenti"', 'totale_area_componenti', 6, 0, 4),
            ('"totale area malta"', 'totale_area_malta', 6, 0, 4),
            ('"rapporto componenti/malta"', 'rapporto_componenti/malta', 6, 0, 2),
            ('"width_min"', 'width_min', 6, 0, 4),
            ('"width_max"', 'width_max', 6, 0, 4),
            ('"width_range"', 'width_range', 6, 0, 4),
            ('"width_mean"', 'width_mean', 6, 0, 4),
            ('"width_stddev"', 'width_stddev', 6, 0, 4),
            ('"height_min"', 'height_min', 6, 0, 4),
            ('"height_max"', 'height_max', 6, 0, 4),
            ('"height_range"', 'height_range', 6, 0, 4),
            ('"height_mean"', 'height_mean', 6, 0, 4),
            ('"height_stddev"', 'height_stddev', 6, 0, 4)
        ])
        ...
        # Layer poligonale (MODIFICATO: rimossa lista campi calcolati dal FIELDS_TO_COPY)
        ...
        # Riorganizza campi layer poligonale (RIMOSSI i campi calcolati)
        geo_fields = self.create_field_mapping([
            ...
            (f'"{FieldNames.AMBIENTE}"', FieldNames.AMBIENTE, 10, 0, 0),
            (f'"{FieldNames.CAMPIONE}"', FieldNames.CAMPIONE, 10, 0, 0),
            (f'"{FieldNames.AREA_CAMPIONE}"', 'area_campione', 6, 0, 4),
            ('"num_componenti_parziali"', 'num_componenti_parziali', 2, 0, 0),
            ('"totale_area_componenti_parziali"', 'totale_area_componenti_parziali', 6, 0, 4),
            ('"num_componenti_interi"', 'num_componenti_interi', 2, 0, 0),
            ('"totale_area_componenti_interi"', 'totale_area_componenti_interi', 6, 0, 4),
            ('"media_area_componenti_interi"', 'media_area_componenti_interi', 6, 0, 4),
            ('"totale_area_componenti"', 'totale_area_componenti', 6, 0, 4),
            ('"totale_area_malta"', 'totale_area_malta', 6, 0, 4),
    ...
    def shortHelpString(self) -> str:
        return """
        <h3>Analisi Quantitativa Altri Componenti - Versione 0.7</h3>
