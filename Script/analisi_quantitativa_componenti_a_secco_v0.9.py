"""
SCRIPT ANALISI QUANTITATIVA COMPONENTI A SECCO - VERSIONE 0.9
"""
...
class Analisi(QgsProcessingAlgorithm):
    ...
    def _log_header(self, feedback):
        ...
        feedback.pushInfo("\n" + "="*70)
        feedback.pushInfo("ANALISI QUANTITATIVA COMPONENTI A SECCO - VERSIONE 0.9")
...
END'''
    ...
    def _create_rilievo_analysis(self, layer_base: str, bbox_layer: str,
        ...
        if layer:
            # Aggiungi variabile piede al layer
            QgsExpressionContextUtils.setLayerVariable(layer, 'piede', 0.296)
            feedback.pushInfo("Variabile 'piede' aggiunta al layer (valore: 0.296)")
            
            # Aggiungi campi virtuali
            from qgis.PyQt.QtCore import QVariant
            from qgis.core import QgsField
            
            # Campo 1: width_piede (solo per superficie = 'intera')
            width_piede_expr = '''CASE 
        WHEN "superficie" = 'intera' THEN floor("width_bbox" / @piede)
        ELSE NULL
    END'''
            layer.addExpressionField(width_piede_expr, QgsField('width_piede', QVariant.Int))
            feedback.pushInfo("Campo virtuale 'width_piede' aggiunto (solo per superficie='intera')")
            
            # Campo 2: width_modulo (solo per superficie = 'intera')
            width_modulo_expr = '''CASE 
        WHEN "superficie" = 'intera' THEN round("width_bbox" % @piede, 3)
        ELSE NULL
    END'''
            layer.addExpressionField(width_modulo_expr, QgsField('width_modulo', QVariant.Double))
            feedback.pushInfo("Campo virtuale 'width_modulo' aggiunto (solo per superficie='intera')")
            
            # Campo 3: height_piede (solo per superficie = 'intera')
            height_piede_expr = '''CASE 
        WHEN "superficie" = 'intera' THEN floor("height_bbox" / @piede)
        ELSE NULL
    END'''
            layer.addExpressionField(height_piede_expr, QgsField('height_piede', QVariant.Int))
            feedback.pushInfo("Campo virtuale 'height_piede' aggiunto (solo per superficie='intera')")
            
            # Campo 4: height_modulo (solo per superficie = 'intera')
            height_modulo_expr = '''CASE 
        WHEN "superficie" = 'intera' THEN round("height_bbox" % @piede, 3)
        ELSE NULL
    END'''
            layer.addExpressionField(height_modulo_expr, QgsField('height_modulo', QVariant.Double))
    ...
    def _log_summary(self, count_interi: int, count_parziali: int, params: Dict,
        ...
        feedback.pushInfo("  * width_piede (intero) - SOLO per superficie='intera'")
        feedback.pushInfo("  * width_modulo (decimale, 3 decimali) - SOLO per superficie='intera'")
        feedback.pushInfo("  * height_piede (intero) - SOLO per superficie='intera'")
        feedback.pushInfo("  * height_modulo (decimale, 3 decimali) - SOLO per superficie='intera'")
    ...
    def shortHelpString(self) -> str:
        return """
        <h3>Analisi Quantitativa Componenti a Secco - Versione 0.9</h3>
        ...
        <ul>
            ...
            <li><b>analisi_rilievo:</b> Rilievo arricchito con metriche bbox e campi calcolati (width_piede, width_modulo, height_piede, height_modulo)</li>
        ...
        <ul>
            <li><b>width_piede:</b> Numero di piedi interi nella larghezza (floor(width_bbox / 0.296))</li>
            <li><b>width_modulo:</b> Resto della larghezza in metri (width_bbox % 0.296, arrotondato a 3 decimali)</li>
            <li><b>height_piede:</b> Numero di piedi interi nell'altezza (floor(height_bbox / 0.296))</li>
            <li><b>height_modulo:</b> Resto dell'altezza in metri (height_bbox % 0.296, arrotondato a 3 decimali)</li>
        </ul>
        ...
        <ul>
            ...
            <li>I campi piede e modulo sono NULL per i componenti parziali</li>
            <li>Il filtro materiali è case-sensitive</li>
            <li>Al layer analisi_rilievo viene aggiunta la variabile 'piede' con valore 0.296</li>
            <li>Il valore del piede (0.296 m) è codificato nello script e corrisponde al piede attico/romano</li>
        </ul>
        
        <p><b>Versione:</b> 0.9</p>
        """
